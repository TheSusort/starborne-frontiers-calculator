    ...
    return (
        <div className={`bg-dark rounded space-y-2 ${className}`}>
            ...
            {Object.entries(stats).map(([statName, value]) => {
                ...
                const difference = comparisonValue !== undefined ? value - comparisonValue : undefined;

                return (
                    <div key={statName} className="flex justify-between items-center">
                        <span className="text-gray-300 capitalize">{STATS[statName as StatName].label}</span>

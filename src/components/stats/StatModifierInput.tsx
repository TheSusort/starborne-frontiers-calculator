...
export const StatModifierInput: React.FC<Props> = ({
  ...
  const handleStatChange = (index: number, field: keyof Stat, value: string) => {
    ...
    if (field === 'name') {
      ...
      const allowedTypes =
        allowedStats?.[value as StatName]?.allowedTypes || STATS[value as StatName].allowedTypes;

      // Filter out excluded types for this stat name
      const validTypes = allowedTypes.filter(
        (type) =>
          !excludedStats?.some((excluded) => excluded.name === value && excluded.type === type)
      );
    ...
    .map(([key, value]) => {
      const allowedTypesForStat = value.allowedTypes?.filter(
        (type) =>
          ...
          !excludedStats.some((excluded) => excluded.name === key && excluded.type === type)
      );
    ...
    <div className="space-y-4">
      {stats.map((stat, index) => (
        <div key={index} className="flex gap-4 items-end">
          ...
          />
          <Button aria-label="Remove stat" variant="danger" onClick={() => removeStat(index)}>
            <CloseIcon />
          </Button>
        </div>
      ))}

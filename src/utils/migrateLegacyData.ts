 ...
 */
export const migrateLegacyData = (): MigrationResult => {
 ...
 * @param migrationResult - The result from migrateLegacyData
    ...
    try {
        ...
        if (inventory.length > 0) {
            ...
            if (validInventory.length > 0) {
                ...
                const inventoryItems = validInventory.map((item) => ({
                    id: item.id,
                    user_id: userId,
                    slot: item.slot,
                    level: item.level,
                ...
                const allGearStats = validInventory.flatMap((item) => {
                    const stats = [];
        ...
        if (ships.length > 0) {
            ...
            if (validShips.length > 0) {
                ...
                // Batch insert implant stats
                if (implantStatsRecords.length > 0) {
                    const { error: implantStatsError } = await supabase
                        .from('ship_implant_stats')
                        .insert(implantStatsRecords);

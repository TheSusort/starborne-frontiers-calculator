/**
 * Tripwire for a test-harness defect that propagates by copy-paste:
 *
 *     beforeEach(() => {
 *         setupKeyedRng(12345);
 ...
 * file-level one. Measured: two identical `setupKeyedRng(12345); resetRateGateRng()` setups
 * produce different draw sequences, while seed-only setups are byte-identical. A probe run under
 * the offending shape reads as "fixtures diverge" when the real cause is an unseeded draw, not
 ...
 * preceding `setupKeyedRng(...)`. It does NOT flag the legitimate seed-run-restore idiom,
 ...
 *
 * The mask also settles which lines can START a scan: a `setupKeyedRng(` inside a comment is a
...
function offendingLines(lines: string[]): number[] {
    ...
    lines.forEach((line, i) => {
        if (inert[i]) return;
        if (!/\bsetupKeyedRng\s*\(/.test(line)) return;
...
describe('rate-gate seeding order', () => {
    it('never calls resetRateGateRng() directly after setupKeyedRng()', () => {

 ...
 *
 * The pair below is deliberately two-sided, the way #318's widened `hasPositionedEnemyRoster` gate
 * had to be fenced both too-strict and too-loose before it could be called the narrowest correct
 * signal:
 *   • TOO LOOSE — if an ACTOR could carry the string, the bucket's `buff-expired` and that actor's
 *     own events would interleave under one id: invisible in the log, impossible to attribute
 *     afterwards. Fenced by the first case.
 *   • TOO STRICT — dropping the reservation is a silent behaviour change, not a cleanup: the
 *     collision above becomes constructible from outside the engine. Fenced by the second case.
 *
 * NON-VACUITY, which matters more here than usual: the first case reads the roster through
...
describe('SP-4c-2d: the sentinel id is reserved in both directions', () => {
    ...
    it('runCombat still REJECTS an enemy attacker named with the sentinel id', () => {
        ...
        // name a real attacker `'enemy'` would interleave its events with the bucket's.
        expect(() =>
            runCombat({
                ...bareInput(),
                enemyAttackers: bareEnemy({ id: SENTINEL_ENEMY_ACTOR_ID }),
            })
        ).toThrow(/collides with a reserved or player actor id/);
    });

    it('...and the rejection is about the SENTINEL, not about every id', () => {
        ...
        expect(() =>
            runCombat({ ...bareInput(), enemyAttackers: bareEnemy({ id: 'not-the-sentinel' }) })
        ).not.toThrow();
    });
});

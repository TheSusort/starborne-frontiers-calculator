 ...
 *  source the rest. Unremovable means a CLEANSE cannot strip it, never that a steal cannot take
 *  it.
 *
 *  Membership routes the transfer through the StatusEngine's per-owner stack LEDGER (a signed
 *  delta folded into `selfBuffStacksForOwner`) rather than through the timed-entry move. That is
 *  a SEPARATE path, which is why it is unaffected by {@link UNREMOVABLE_STATUSES}: the steal's
 *  timed-candidate loop still skips every unremovable name, Protection included, and a
 *  stack-stealable status is not a timed entry to begin with.
 *

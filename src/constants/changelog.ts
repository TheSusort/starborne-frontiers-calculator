...
export const UNRELEASED_CHANGES: string[] = [
    "Combat simulator: debuff landing and resist now respond to Hacking and Security buffs during the fight — a ship hit with Hacking Down lands fewer debuffs, and a target with Security Up resists more — and affinity advantage/disadvantage now adjusts a ship's landing chance (in addition to its existing effect on damage). The chance is recomputed each turn against the actual target rather than fixed at the start of combat.",
    'Combat simulator: turn order now responds to Speed Up and Speed Down buffs — ships buffed with Speed Up act earlier in the round and ships with Speed Down act later, with the order updating mid-round as speed changes take effect. Ships with conditional extra actions (Liberator, Nuqtu, Sustainer, Tormenter, Tygr) re-enter the turn queue at their current Speed, including any live speed buffs. Ships with an end-of-round extra action (such as Harvester) take that extra turn at the very end of the round, after every other ship has acted, regardless of Speed.',
    'New Combat Simulator: place two squads on the hex board and watch a round-by-round battle play out — both sides are your own fleet ships using their geared stats. Step through each round to see per-ship HP, damage, and healing on the board, pin a ship for a detailed per-round breakdown, and read the turn-by-turn event log — now a chronological play-by-play with turn markers, team-labeled (enemy ships are prefixed with "Enemy") and color-coded, showing who hit whom for how much alongside heals, buffs, debuffs, damage-over-time afflictions, and deaths — with the final outcome called out at the top. You can also save and load board setups as encounters: save the team you build on either board as a new encounter, then reload it later from the Load dropdown.',
    ...
    "Major DPS Calculator engine upgrade: ship skills are auto-parsed into an editable ability model, and the simulation now runs a real combat engine — speed-ordered turns for team ships and the enemy (with a per-config Turn Order strip), live conditional gates, application-time debuff landing with persistent windows, buff-family tier rules, charge cadences with passive auras, and reactive triggers (charge-on-inflict, crit-triggered effects, start-of-round buffs) firing from real combat events. Includes numerous accuracy fixes (Valerian, Hemlock, Oleander, Enforcer, and persistent stacking debuffs like Defense Shred that climb per hit and never expire). Team ships are now full combat actors too: pick a ship and its real skills are parsed and simulated — own stats and affinity, real damage shown separately as Team damage (reduces enemy HP without polluting the attacker's DPS comparison), DoTs and debuffs on their actual turns, reactive triggers, and ally-wide buffs and charge grants for the whole team. With a team configured, the round chart adds a dashed combined-total line carrying the kill mark at the round the enemy actually dies, with a dashed legend swatch to tell team lines apart from the solid attacker lines. Combat Settings is now split into separate Enemy Settings and Team sections you can open independently. Multi-hit skills (such as Enforcer's three-hit active) now crit-check each hit individually — crit-triggered effects like Defense Shred fire once per critting hit rather than once per turn, matching in-game behaviour. Ships with extra-action passives (Liberator, Nuqtu, Sustainer, Tormenter, Tygr) take a full additional turn each round, re-entering the turn order at their Speed position; rounds where an extra turn occurs show a '+N extra turn' line in the round chart tooltip. Ships that react when an ally lands a DoT with a critical hit (Crocus, which inflicts Corrosion II on that enemy) now trigger correctly in team simulations. Chakara's start-of-round Attack Up / Defense Up self-buffs are now simulated when it has the lowest Speed on its team.",
    "Rebuilt Healing Calculator on the same deterministic combat engine as the DPS Calculator. Heal, shield, and cleanse abilities are now auto-parsed from each healer's skill text into an editable ability model — open the Skill Editor to tune heal percentages, scaling stat, shield amounts, and cleanse counts per skill. Pick a heal target (the healer itself, another ship, or manual stats) and add an enemy team of any size — real ships whose charged skills produce realistic damage spikes, or quick manual stat lines — that bombard the target round by round. Results separate effective healing from overheal, track a shield absorption pool that drains before HP (capped at the target's max HP), tick heal-over-time effects (Repair Over Time) at the holder's turn, and resolve crit heals on their own crit schedule. A survival metric reports how many rounds the target stays up — or the round it is destroyed. Team healers contribute their own heals, shields, and cleanses, and reactive heal triggers (such as Pallas reacting to critically repairing an ally) fire from real combat events. Repairs without an explicit recipient on pure support skills (Hermes, Mender, Salvation, Makoli and 13 more) correctly heal the targeted ally rather than the caster — while damage-skill repair riders and self-conditional heals like Meatshield's stay self-heals. Passive repairs triggered by an ally taking damage, or by a Supporter cleansing a debuff (Cultivator), likewise heal that ally, while a Defender's cleanse-repair (Morao) stays a self-heal. Ships that repair or gain shields based on a percentage of damage dealt or taken — lifesteal-style sustain (including from damage-over-time ticks), burst-triggered heals, and shield-on-hit — are now simulated too. Ships that gain a shield when an enemy is debuffed (APEX) or when they apply Stasis (Defiant) now react to their own actions, and ships that grant themselves a repair buff alongside another effect (Hermes' Everliving Regeneration) now have that buff picked up automatically. Team healers expose an editable Heal Modifier so their heal output reflects their real heal-boost stat. Skill auto-parsing now reads only a ship's active refit passive, so it no longer double-counts buffs from inactive refit tiers. Enemy attackers now act as full combat actors on the same pipeline as your ships — their affinity is matched against the heal target's affinity to scale incoming damage, they apply their real debuffs and DoTs to the tank (which tick for damage each round), and buff themselves with their own abilities. A new Enemy Effects panel shows, round by round, the effects grouped by each enemy attacker (labelled with its ship name): the self-buffs active on that enemy and the debuffs or DoTs it has landed on the target. Like the DPS engine, everything is fully deterministic — identical inputs always produce identical results.",
    ...
    'Cleanse abilities now actually remove debuffs (newest first, keeping unremovable statuses like Acidic Decay) in the healing and battle simulators, instead of only counting them.',
    'Purge skills now actually remove buffs from the targeted enemy (newest first, keeping unremovable buffs like Magnetized Shielding) in the battle simulator, instead of being unsimulated.',
    'Purge now triggers reactions in the battle simulator: Sefuba repairs itself and purges an additional buff when it purges an enemy, and Salvation repairs an ally for part of its own Max HP whenever that ally has a buff purged.',
    'Battle simulator: more purge skills now fire from their trigger conditions. Iridium purges its attacker when directly damaged (1 buff, or 2 on its later refit); Rhodium purges the most-buffed enemy at the end of each round (2 buffs); and Faust purges whoever kills it — but only on a killing blow from a direct attack (2 buffs, or 3 on its later refit), so a death from damage-over-time ticks no longer triggers it.',
    "Combat sim: Nayra's charged ability now purges enemy buffs only when its target was repaired that round (previously it removed all buffs every cast).",
    'Healing Calculator: ship deaths and Cheat Death are now simulated. Ships with a recognised Cheat Death buff (Yazid, Tycho, Hayyan) survive an otherwise-lethal hit at 1 HP once per battle, clearing their removable buffs and debuffs as it triggers; Yazid then repairs 60% of its Max HP and gains Barrier in the same round. Once a ship has spent its Cheat Death, the buff chip no longer shows as active in later rounds — it only appears while a save is still possible. Effects that fire on a ship being destroyed now resolve in the round the death happens — Salvation repairs all allies when it is destroyed, and on-kill / on-ally-destroyed reactions are modelled too (Sokol and Liberator take an extra action after destroying an enemy, Liberator also charges every ally on a kill, and Harvester gains an extra action when an ally is destroyed).',
    "Healing Calculator: reactive and triggered repairs now scale with the recipient's Incoming Repair and the caster's Outgoing Repair, exactly like cast repairs do. A Cheat Death repair on a unit carrying Everliving Regeneration (e.g. Yazid) now benefits from its +20% Incoming Repair, instead of being under-counted.",
    "Healing Calculator: a destroyed ally no longer inflates the reported total of its own on-destroyed group repair — Salvation's all-allies repair on death now credits only the living allies it reaches, not a phantom share for the dead caster.",
    "Healing Calculator: ships now react per hit when directly damaged — Warden repairs 3% each hit and inflicts Corrosion on the attacker, Isha repairs 3% on a normal hit and 6% on a crit hit, Makoli and Guardian repair 20% when hit below 40% HP (Guardian also gains Binderburg Resilience when critically hit), and Heliodor repairs 8% on every hit. Team supports also react when the tank is hit: Cultivator repairs the damaged ally for 8% of its own Max HP, Refine grants the damaged ally Inc. Damage Down, Graphite grants Repair Over Time when the damaged ally is an Attacker or Debuffer (ship roles are read from your ship data automatically), Guardian Provokes an enemy that critically hits an ally, and Heliodor repairs all allies when it is directly damaged. Several ships' on-hit reactive buffs and debuffs — Shepherd, Opal, Flamel, Iridium, Panguan, and Stalwart — no longer incorrectly apply every round; they now only fire when the ship is actually hit. The Skill Editor gains a 'When an ally is attacked' trigger with an ally-role filter, and the crit/non-crit hit filter is editable on both the 'When attacked' and 'When an ally is attacked' triggers. Ships that grant a protective effect when their HP drops below a threshold now trigger on real HP drops instead of being treated as always-on: Tycho, Shelter, and Los grant Barrier the first time they fall below 40% HP (once per battle), and Kafa (Terran Tenacity) and Redeemer (Defense Up) re-apply their defensive buff each time they cross the threshold downward. Barrier is now a real defensive buff rather than display-only: while active it blocks all incoming damage — attacks, damage-over-time, and bombs alike — shown as a new Barrier Absorbed total. Because it is granted the instant a ship drops below its HP threshold, it shields against the remaining attacks that round, so its protection grows with the number of enemy attackers. Hermes' charged-skill Cheat Death now only triggers when its heal target is below 40% HP, and protects that ship rather than the whole team — so Hermes joins the recognised Cheat Death ships (Yazid, Tycho, Hayyan, and now Hermes); Hayyan's Cheat Death grant now applies when its charged skill fires. Ships now also react to an enemy repairing or cleansing a debuff: Zosimos banks a charge toward its Charged Skill whenever an enemy repairs, and Arum, Yarrow, Larkspur, and Grif react when an enemy cleanses a debuff — Arum weakens the cleansing enemy (and grants its team Gelecek Contagion), Grif retaliates with damage, and Yarrow and Larkspur buff themselves. Skill effects that depend on the enemy having a buff, or on a ship being Provoked or Taunted, now reflect the real battle state instead of always assuming the condition is active — so 'deals more damage to enemies with Stealth' or a Provoke-gated debuff only takes effect once that buff or control effect is actually present, not on round one of a clean fight. Enemy attackers can now carry a Hacking stat: their debuffs (both timed effects and damage-over-time) now land based on the attacker's Hacking versus the heal target's Security, instead of always landing — the heal target gains a new Security input for this. New manual enemies default to 200 Hacking, and picking an enemy ship fills it from the ship's stats; raise the heal target's Security to reduce how often enemy debuffs land. When an enemy debuff or damage-over-time effect is resisted it is now shown in the round breakdown (greyed out and marked 'resisted'), so you can see what missed instead of it silently disappearing. Enemy attackers now also fire their own reactive self-buffs — for example Chakara's start-of-round Attack Up / Defense Up when it is the slowest enemy — so incoming damage reflects the buffs an enemy grants itself during the fight, not just its base stats. The enemy section is now a full Enemy Team (no longer capped at four): enemy support ships buff their teammates, so a buffer's Attack Up raises the damage the other enemies deal to your heal target, and a support ship with no attack of its own simply buffs the team without dealing damage. Ships that grant a charge to their whole team are now simulated too — Hayyan's charged skill and Graphite's start-of-round passive (when an enemy has Stealth) add a charge to all allies' Charged Skills, speeding up their bursts; configured as enemy attackers they likewise accelerate the enemy team's charged skills. Provoke now redirects a provoked attacker to target whoever applied it — both skill-cast and reactive appliers (such as Guardian's counter-Provoke on a critically hit ally); ships that 'ignore Taunt and Provoke' bypass that forced targeting, while Concentrate Fire still applies.",
];

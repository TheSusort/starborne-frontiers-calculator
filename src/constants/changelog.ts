...
export const UNRELEASED_CHANGES: string[] = [
    ...
    'Autogear: a new "Assume all gear is calibrated" per-ship setting scores every 5★/6★ piece as if it were calibrated to the ship being optimized, instead of only pieces you have actually calibrated to it (or left uncalibrated) competing on their real stats. Turn it on to see a gear piece\'s ceiling — what it would offer if you calibrated it here — regardless of which ship currently holds its calibration or whether it holds none at all. Both the suggested loadout and the current-loadout comparison use the same assumption, so the reported gain reflects only the gear swap, not a calibration you could apply either way. Off by default; existing runs are unaffected.',
];

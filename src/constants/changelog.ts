...
export const UNRELEASED_CHANGES: string[] = [
    'Combat sim: turn order now uses the real in-game speed tiebreak. Ships with equal Speed are ordered by board position — the whole top row before the whole middle row before the bottom row, and within a row the backmost cell first — and only a genuine position tie (the same cell on both sides) falls back to your team going first. Previously equal-Speed ties were resolved by roster order, which could run your ships in the wrong sequence.',
    'Combat log: when a follow-up proc (e.g. the Insidiousness implant) triggers an emergency shield or breaks one, those lines now appear beneath the attack that caused them instead of above it.',
    'Combat sim: a skill that detonates a damage-over-time effect AND re-applies that same effect (e.g. a Bomb detonator that also plants a Bomb, or Incinerator with Inferno) no longer detonates the stack it just applied. In team battles this meant the new stack was consumed instantly by its own cast, so a Bomb planted this way could never survive to count down and burst, and the first cast paid out damage it should not have. Single-target/DPS-mode casts were already correct.',
    'Combat log: the Lifeline implant\'s emergency shield (granted mid-hit when a hit would drop the ship below 30% HP) now shows a shield line. Previously the shield pool appeared out of nowhere — and if the same hit broke it, the log showed a "shield destroyed" for a shield it never showed being granted.',
    "Combat sim: the buff/debuff list you see when clicking a ship now reflects effects being REMOVED. Previously it only ever added, so a debuff that was cleansed, purged, stolen, or that simply expired stayed listed on the ship for the rest of the battle — which contradicted the combat log (e.g. AEGIS cleansing a shield-broken ally's debuffs, then the ally still showing them).",
    'Combat log: shield generated at the start of a ship\'s turn (the Shield gear set\'s "Generate 4% shield each turn") now appears under that ship\'s turn instead of being filed at the end of the round, where it read as a shield appearing out of nowhere — and later made "shield destroyed" look like a shield that never existed.',
    ...
    "Combat sim: AEGIS now reacts to its OWN shield being destroyed (granting itself Defense Up and cleansing all its debuffs), not only an ally's — its support pattern is centered on itself, so it counts as an ally within the pattern.",
    'Combat log: reactive cleanses (e.g. AEGIS cleansing an ally when its shield is destroyed) now show a log line attributed to the reacting ship — previously the cleanse happened but was invisible in the log, making it look like nothing reacted.',

...
export const UNRELEASED_CHANGES: string[] = [
    ...
    'Defense calculator: Theoretical EHP no longer counts a kit buff whose condition is not currently met, and the card now names what it left out and why. Redeemer at 40,000 HP and 5,000 Defense used to read 113,250 — that total included Defense Up II, which only arrives once Redeemer drops below 60% HP — and now reads 96,025 at full health, with a line under the figure reading "Not counted (conditional): Defense Up II - below 60% HP". Buffs you picked yourself in the buff picker are unaffected; this only changes buffs auto-filled from a ship’s own kit. One knock-on: on a page with no enemies configured, where "Best ship configuration" and the chart’s ship markers both fall back to Theoretical EHP because there is no measured fight to rank by, the badge can now land on a different card than before for the exact same inputs — both read the same, now more honest, figure.',

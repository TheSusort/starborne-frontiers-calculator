...
export const CHANGELOG: ChangelogEntry[] = [
    {
        ...
        date: '2026-09-10',
        changes: [
            'Autogear: Custom mode replaces Manual — build a role out of the stats you pick, each aiming high or low and multiplying or adding, and copy a built-in role into an editable formula to start from. Stat priority rows lost their reorder arrows, since nothing read the order.',
            'Autogear: a new Direct Damage stat combines attack, crit, crit power and defense penetration, and both it and Effective HP can now be used as stat bonuses, not only as requirements. A stat bonus previews the score it adds before you save it, and heal modifier and shield scale bonuses no longer swamp the score.',
            "Game Basics: a new page explaining stats, roles, affinities and targeting, with an interactive board showing which ships each targeting rule hits and how a pattern clips at the board edge, and an affinity wheel that isolates one affinity's matchups on hover. Every stat and gear set row is directly linkable.",
            "Game Basics and calculators: the debuff landing formula now names the affinity hacking swing of 25%, in Game Basics, the documentation and the healing calculator's help text.",
            "Upgrade analysis: a new coverage grid shows farming priority by role and slot, with an adjustable number of target pieces per slot, colours scaled to your own grid's range, and the Role Filter narrowing it as it does the rest of the page. It can be collapsed to save space, and role cards and slot tabs lead with the highest priority.",
            "Upgrade analysis: priority now compares a piece against a typical good piece rather than the best one possible, and excess crit rate no longer inflates a piece's value or piles up in the grid's ideal pieces.",
            'Upgrade analysis: coverage grid preferences are now kept per account and restored when you sign back in.',
            'Ship inventory: your list or image view choice now survives a reload and comes back when you sign in again.',
            'Factions: the Tianchao faction is now called Tianchen, matching the game — searching either name finds its ships.',
            'Skill cards: targeting descriptions now say which row a skill hits.',
            'Skill editor: the named-status field now accepts multi-word names.',
            'Combat simulator and calculators: another correctness pass on the shared combat engine — roughly two dozen fixes spanning Protection stealing, repairs, leech and over-repair redirects and the passives that key off them, stacking buffs and adjacency reach, damage converted into damage over time and the damage-taken figure reactions read, skills that scale with the board count, and debuff durations. The round log now names buff steals and says what a repair or shield wasted.',
        ],
    },
    {
        version: '1.66.0',

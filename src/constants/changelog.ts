...
export const UNRELEASED_CHANGES: string[] = [
    ...
    'Healing calculator: the enemy team can no longer be emptied — the last enemy keeps its place. With no enemy at all the fight fell back to an internal stand-in target with fixed Defense, which silently changed every repair scaled off the damage your healer deals (measured about three times too small). Enemies you pick now also use their own real targeting, so an enemy with an area attack hits everything its pattern covers instead of only one of your ships, and spreading your team out matters against them too.',
    ...
    'DPS calculator and combat simulator: a passive that deals damage on its own — Judge’s "at the start of the round, deals 60% damage to all enemies below 50% HP" is the clearest example — now actually lands. The damage was worked out and then thrown away, so the round it should have jumped in, the total simply did not move. It now hits the enemies the passive itself names rather than whatever your attack happened to hit: an "all enemies" passive reaches the whole enemy board even when your skill is single-target, and a single-target passive stays on your main target even when your skill is an area attack. Enemy ships with such a passive do the same to you.',
];

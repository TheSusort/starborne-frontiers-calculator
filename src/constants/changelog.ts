...
export const UNRELEASED_CHANGES: string[] = [
    'Defense calculator: “Measured EHP” now counts the raw damage thrown at a ship, so a tankier ship finally scores HIGHER instead of lower. It used to add up the damage that got THROUGH — after Defense had already reduced it — which quietly inverted the whole page. A ship that survives leaks less the tankier it is, so it scored WORSE; a ship that dies soaks roughly its own HP no matter what its Defense is, so the number barely moved at all. Isha was reporting 1,408 next to a formula estimate of 543,950 — those were never two estimates disagreeing, they were two different quantities. Concretely: a ship with 100,000 HP under 60,000 damage a round now reads 120,000 at zero Defense, 300,000 at 5,000 Defense and 720,000 at 20,000 Defense, matching how much longer it actually lasts (2, 5 and 12 rounds). The figure now measures the same thing the Effective HP formula beside it estimates, so when the two disagree that disagreement finally tells you something. Two things worth knowing when reading it. The number moves in whole rounds, because it only grows when the ship survives another round — two ships that die on the same round show the same figure even if one is a little tankier, which is why rounds survived now sits beside it. And for a ship that survives the whole window it is a lower bound, not a limit: it is simply everything that was thrown, so it stops rising once nothing is left to throw. The breakdown underneath (to hull, absorbed by shield, blocked by Barrier, converted to shield) still describes what actually reached the ship after Defense, and is now labelled as such — those rows are not meant to add up to the headline.',

...
export const UNRELEASED_CHANGES: string[] = [
    "Combat simulator: supporter heals, shields, buffs, and ally charge grants now respect the ship's friendly skill pattern — only allies standing on that pattern's footprint receive them, not the whole team. Reactive passives (such as start-of-round charge grants) use the same rule.",
    "Combat simulator: co-cast shields beside an all-allies buff grant (such as Graphite's Overclock + shield active) now shield every ally in the support pattern, not just the caster.",
    "Combat simulator: the team boards now show each ship's remaining shield pool on its cell (when shields are active), alongside the HP bar.",
    'Combat simulator: the HP bar and pinned ship view now reflect actual HP remaining after shields absorb damage, instead of treating shield-blocked damage as HP loss. A ship that took a big hit mostly absorbed by shields no longer shows 0% HP while still alive.',
    'Combat simulator: area-of-effect attacks now roll critical hits per target instead of sharing one roll across every ship hit. Each target is checked against the attacker’s own affinity matchup with it, so a target the attacker is at an affinity disadvantage against crits less often (its crit chance is capped and penalised), while a neutral or advantaged target is unaffected — matching how the game resolves each target independently. Effects that key off critical hits follow suit: the Bloodthirst implant now rolls its heal per critting target, and on-crit effects fire when any target is critically hit.',
    'Combat & DPS simulators now use real random chance for critical hits, debuff landing (hacking vs security), and chance-based procs, instead of a fixed deterministic schedule. Results now reflect natural variance — for example the first hit can now land a critical hit at high crit rate, which the previous always-second-hit scheduling never allowed. The DPS calculator figure will vary slightly between runs as a result.',
    "Combat simulator: the Martyrdom implant's Disable now lasts its full duration on the killer — a legendary Martyrdom disables the killer for two turns (rare for one). Previously the killer's own turn ended immediately after the kill ate the first turn of the Disable, so it was cut short by one turn.",
    'Combat log: charge changes caused by a reaction (such as an on-enemy-death "all allies add a charge" passive) are now correctly shown nested under the action that triggered them, instead of appearing as a separate top-level turn action. This also fixes other reactions on the same turn (like a destroyed ship\'s Disable on its killer) from occasionally being mis-attributed under that charge change.',
    ...
    'Combat simulator: in positioned battles, area-of-effect attacks now trigger the on-being-hit reactions of every ship caught in the blast — counter-attacks, retaliation buffs, and on-hit repairs — not just the primary target. Applies to both your fleet and the enemy.',
    'Combat simulator: the event log is now fully detailed — every action, reaction, and effect is recorded. Area-of-effect attacks and heals list every affected ship with its individual damage or heal amount and resulting HP. Reactions such as counterattacks, on-crit heals, reflects, and reactive shields now appear nested under the turn that triggered them rather than being attributed to the reacting ship\'s own turn. Per-ally heal amounts are now accurate for each recipient. Charge state is shown each turn (e.g. "charge 2/3"), with active and charged skill variants tagged, and explicit lines for charge gains, resets, and manipulations.',
];

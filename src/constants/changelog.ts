...
export const UNRELEASED_CHANGES: string[] = [
    ...
    'Combat simulator: Exposed now does something. It used to be a name in the debuff list with no effect at all — it now increases the next direct hit on that ship by 100% per stack, and each stack arms one hit. Amartya applies 2 stacks, so the next hit lands at +200% and spends one stack, the hit after that lands at +100% and spends the last one, and the third is normal. Applies to both sides, so an enemy Exposed makes your ship take the extra damage too.',
    ...
    "Combat simulator: Hit Mitigation now does something. Oleander's charged skill gives it to the whole team, where it blocks the next direct hit on each ship and spreads that damage over the ship itself across three rounds instead. It used to be a name in the buff list with no effect at all. A hit that something else already stopped leaves it armed for the next real one: a Barrier, a damage-into-DoT transform (Voron, Orel), a Bomb going off and DoT ticks all pass it by. Applies to both sides.",
    "Combat simulator: one-shot statuses are now only counted when they can actually be used up. Exposed and Toxic Overflow picked by hand in a buff or debuff picker no longer apply at all, because a hand-picked status carries no turn count and so could never be spent — a hand-picked Toxic Overflow used to spread Corrosion to adjacent allies every round for the whole fight instead of once, and a hand-picked Exposed amplified every hit instead of one. Hit Mitigation follows the same rule. All three still apply in full when a ship's skill grants them.",
    ...
    "Combat simulator: Out. Detonation Damage Up now does something. Chimei grants it to an ally, where it used to be a name in the buff list with no effect — it now increases that ally's detonation damage by 45%, both for Bombs they plant and for Corrosion or Inferno they set off.",
    ...
    'Combat simulator: when a reflect or a counterattack is blocked and spread over time (by Hit Mitigation, or a damage-into-DoT transform such as Voron or Orel), the bounce is no longer also reported as damage the moment it happens. It used to be counted twice — once as immediate damage taken and dealt, and again as it ticked. Health totals were always right; only the reported damage numbers were inflated.',
    'Combat simulator: damage numbers now add up when Protection redirects part of a hit. The redirected part was counted on both the protected ship and the protector, so the attacker appeared to deal more than the hit it landed (a 1000 hit reporting 1277) and the protected ship was credited damage its ally took. The protected ship now shows only what actually reached it. If a Barrier stops the redirected part, it is reported like any other barriered hit — the protector no longer shows "0 damage taken" alongside a barrier absorption, and the combat log now includes the redirect line that explains it. The same rule now covers damage a ship blocks with an incoming-block ability and bomb splash onto neighbours. Health totals were always right.',
    ...
    "Combat simulator: Quixilver's refit passive now works correctly. At the end of its turn, if its Shield is at 100% of its max health, it grants the whole team a Barrier good for exactly 1 hit and puts everyone on a 3-turn Barrier Recharging cooldown; once that cooldown runs out, the next full-Shield turn grants a fresh Barrier. Previously its condition and hit limit were not read at all, so it applied as an unconditional, permanent Barrier on the whole team for the entire fight regardless of Shield level — a team with Quixilver was effectively unkillable in the simulator.",
    'Combat simulator: a Barrier granted "for 1 hit" is now used up by the hit it blocks instead of lasting the rest of the fight. This affects Malvex, Panon, Quixilver and Sansi. Barriers granted for a number of turns are unchanged, and neither a Bomb going off nor a DoT tick uses up a hit charge.',
    'Combat simulator: Barrier Recharging (the "cannot be granted Barrier" cooldown from Quixilver and Panon) now blocks a fresh Barrier no matter where it comes from, not only a re-application of the same grant. The knock-on effects: Panon\'s own reactive Barrier (gained when directly damaged) is now limited to once every 3 turns instead of every single hit, and a charge-skill Barrier (Malvex, Sansi, Panon) or the Last Stand implant\'s clutch Barrier is silently skipped if the recipient is still on cooldown — for example, from a Quixilver teammate.',
    ...
    'Combat simulator: enemy support ships now charge and cast their charged skills. Any enemy whose charged skill only heals, shields, buffs or cleanses — with no damage on it — never gained a single charge and so never cast it, for the whole fight. That covered 25 ships, essentially every dedicated support in the game (AEGIS, Chimei, Faust, Mender, Purifier, Salvation, Shelter and the rest). Facing them is now meaningfully harder, and the same ships were always unaffected on your own team.',
    "Combat simulator: counterattacks now answer every hit of a multi-hit attack instead of only the first. A multi-hit skill is a series of separate attacks, so Enforcer's three-hit active draws three counters out of Stalwart, Nyxen or Centurion rather than one — roughly tripling what a countering defender gives back against a multi-hit attacker. Both sides are affected. A single area attack still draws one counter from each ship it lands on, as before. The healing calculator sees the same change when an enemy there uses a multi-hit skill; the DPS calculator is unaffected, because nothing ever attacks your ship there.",
    ...
    "Combat simulator: effects that repair a share of the damage you deal (such as Bloodthirst) now use the damage the attack actually landed — after crits, after damage amplification, and after the target's defence — instead of an idealised pre-mitigation figure. Damage redirected to an ally by Protection still counts toward the repair, while damage converted into a Damage over Time effect no longer does. These effects also now trigger once per attack rather than once per enemy hit, so an area attack produces a single, larger repair instead of several small ones.",
    "DPS calculator and healing mode: a multi-hit skill's own outgoing reactions now split across each of its separate attacks, matching the battle simulator. Burner's Inferno now stacks once per attack (three times on Enforcer's three-hit active, not once), and reactions to a team-mate's critical hit — such as banking extra charge — now fire once per critting attack instead of once for the whole cast, so a team-mate with a 2-hit critical skill grants 2 charges rather than 1. In healing mode, a Bloodthirst-style self-repair now also scales off each attack's own damage instead of the whole cast's total.",
    "Combat simulator: effects that trigger on dealing damage (Burner's Inferno, Warpstrike's debuff-duration reduction, Zeolite's purge) no longer fire off an attack that delivered nothing. They were gated on the damage figure shown before the target's defences were applied, so an attack entirely converted into a Damage over Time effect (Voron, Orel), or fully stopped by an incoming-block ability, still paid out in full. They now read what the attack actually delivered. An attack soaked by a Shield, nullified by a Barrier, or redirected to an ally by Protection still counts as delivered, since that hit did land on-screen. Both sides are affected. The DPS and healing calculators are unchanged: what an attack delivered is only tracked in the positional battle simulator, so those two keep reading the same figure they always did.",
    'Combat simulator: a passive repair that repaired nothing no longer opens a line in the combat log. A repair scaled off the damage you deal (Bloodthirst) works out to zero when the attack that triggered it delivered nothing — converted into a Damage over Time effect, or fully stopped — and the log still showed a "repaired 0" line for it. Shield versions of the same passives were already silent in that case. Passives that punish an enemy for repairing (Ruiner\'s Bomb) also no longer count a zero repair as a repair, in the simulator and in the healing calculator. Repair totals are unaffected, and the DPS calculator does not run repairs at all.',
];

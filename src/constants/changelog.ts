...
export const UNRELEASED_CHANGES: string[] = [
    'Shields gained by converting damage now count as applying a shield, so implants that react to shielding finally fire off them. Malvex and Quixilver both turn a share of the damage they take into Shield — 15% for Malvex, 25% for Quixilver once a hit has punched through her existing pool — and the pool really did appear, but nothing was told about it. Run Quixilver with Resonating Fury equipped and take a hit that breaks through into her health: she now rolls for Crit Power Up 3 on that hit, where before the Shield went up and the buff never came, with nothing in the combat log to explain why. Those conversions also show up as their own combat-log rows now, so you can see the Shield arrive instead of only seeing it break later. Shields granted by a skill you cast were already working and are unchanged.',
    ...
    'Isha and Nayra no longer get their partner\u0027s Affinity Override on the DPS and healing calculators when that partner is not actually on the team. Both kits read \u0022if the other one is on the same team\u0022 \u2014 Isha gains Defensive Affinity Override if Nayra is beside her, Nayra gains Offensive Affinity Override if Isha is \u2014 and the calculators had no way to check, so they assumed the answer was yes and always granted it. Put Nayra on the DPS calculator by herself and her damage was reported as though Isha were standing next to her. The two pages now read the real roster, so the second Override appears only when you actually add the named partner. The battle simulator already worked correctly and is unchanged. If you are comparing a MANUAL configuration \u2014 stats typed in by hand with no ship picked \u2014 nothing changes: there is no ship to ask about, so the gate is still treated as met, exactly as before.',
    'On-resist reactions now treat every strike of a multi-hit skill as its own attack. The clearest case: an Enforcer attacking three times crits and inflicts Defense Shred on each strike, and a Vindicator resists all three \u2014 Vindicator now retaliates three times for 30% of its Max HP each, where before only the first counted. A single attack that applies two debuffs still retaliates once, which was already correct and is unchanged. Applies on both your ships and the enemy\u0027s.',
    'Xcellence\u0027s on-resist passive now fires when ANY enemy resists a debuff, not only when it resists one Xcellence herself inflicted. Her skill reads \u0022When an enemy resists a debuff infliction\u0022 \u2014 nothing in it ties the debuff to her \u2014 but the simulator was treating it as her own debuffs only, so a teammate\u0027s resisted Speed Down did nothing. It now deals its 115%-of-current-Shield hit to each enemy that resisted. Three details match the game: it fires once per resisting enemy per attack (one cast resisted by three enemies procs three times, one hitting each; two debuffs resisted by the same enemy in one attack proc once); it fires again on a later attack in the same round, where it previously stopped after the first; and it does NOT fire on a resist that never rolled \u2014 an affinity-disadvantage application or a Block Debuff auto-resist. Vindicator\u0027s own on-resist passive picks up the same once-per-attack correction. Applies on both your ships and the enemy\u0027s.',
    'The Defense calculator no longer accepts a negative HP, Defense or Security. Typing a minus figure into any of the three used to be committed as-is, and a negative Defense made Damage Reduction, Theoretical EHP and the HP Multiplier all read \u0022NaN\u0022 instead of a number. Those fields now snap back to 0. Clearing the HP field is still allowed — it just no longer turns the HP Multiplier row into \u0022NaNx\u0022.',
    'Opening the Defense calculator from a ship page link now fills in that ship\u0027s own skill buffs, the same as picking the ship by hand on the calculator does. Before this, a defender you arrived at by link had no Defense Up, no Inc. Damage Down and nothing else from its kit, so its Theoretical EHP was understated and the \u0022Not counted (conditional):\u0022 note never appeared — the ship looked like it had no conditional kit at all. The same ship now gives the same answer whichever way you got to the page.',
    'Shield-granting skills now register even when the ship they target already has a full Shield. Previously a Shield grant that landed on an ally whose Shield pool was already at its cap counted as nothing having happened, which silently switched off anything that reacts to a Shield being applied — Resonating Fury being the main one. So a support ship shielding a well-shielded front line would stop granting its Crit Power buff from the moment the pool topped out, with nothing in the combat log to explain it. The grant is now recorded, the reaction fires, and the portion of the Shield that was wasted against the cap is tracked separately — the same way an over-heal on a full-health ship still counts as a repair. Applies to the battle simulator, the healing calculator and the DPS calculator, on both your ships and the enemy\u0027s.',
    'The custom skill editor now only offers targets that make sense for the kind of effect you picked. A Buff can no longer be pointed at enemies, and a Damage or Debuff effect is no longer offered your own allies. Pointing a Buff at \u0022all enemies\u0022 used to look like it would work and then quietly land on a single enemy instead of the whole enemy team. Effects that genuinely work in both directions \u2014 Charge, Control (Taunt targets your own ship) and Extend Status \u2014 keep both sets of targets. If you already saved an effect with a target that is no longer offered, the editor still shows it and marks it as not valid for that effect type, so nothing is silently rewritten and you can change it yourself.',
    'Effects that pick "the enemy with the most buffs" no longer pick a ship that is already destroyed. Two ships use that targeting: Rhodium (purges the most-buffed enemy at the end of every round) and Lodolite (purges it on its charged skill). A destroyed ship keeps the buffs it was carrying when it died and never takes another turn to tick them down, so it could keep winning the \u0022most buffs\u0022 count for the rest of the fight \u2014 the purge landed on the wreck and the living, buffed enemy still shooting at you kept everything. They now pick the most-buffed LIVING enemy, and do nothing at all if every buffed enemy is dead. The similar \u0022highest attack\u0022 (Selenite) and \u0022highest speed\u0022 (Chakara) effects already skipped the dead and are unchanged. Note that a ship hidden by Stealth can still be picked by all of these effects: Stealth hides a ship from being chosen as an attack target, not from a fleet-wide \u0022highest/most\u0022 search.',
    'Retaliations and reactive damage procs now respect outgoing-damage buffs and debuffs, like every other attack already did. Two things were missing. An Attack Down or Out. Damage Down on the retaliating ship was ignored, so a ship that had just been hit with Attack Down II retaliated at full strength anyway. And the ship\u0027s OWN Out. Damage Up never reached its retaliation either \u2014 put Grif (grants all allies Out. Damage Up III) next to Centurion (retaliates when it or an adjacent ally is damaged) and only Centurion\u0027s normal attacks got the bonus, never the retaliation, even though Attack Up from the same cast did. Both are fixed at the retaliation path and at every on-crit / on-resist / on-debuffed damage proc. As everywhere else, the stronger instance of a family wins rather than the two adding up.',
    'Debuffs that reduce crit rate, crit power, speed, hacking or security now actually do something when the OTHER side inflicts them. Until now these five families landed, showed up in the buff list and ticked down correctly, but changed nothing at all — an enemy Speed Down II on your 500-speed ship left it at 500, and a Crit Rate Down III on a 70%-crit enemy left it critting at 70%. Seventeen ships are affected, including Curator, Nayra, Arum, Guardian and Bayah (Crit Rate/Power Down), APEX, Bizon, Torcher, Xcellence, Iridium, Flamel, Rys and Anjian (Speed Down), and Provider and Tygr (Hacking/Security Down). Expect real changes in the simulator: turn order shifts when a Speed Down lands, crit-reliant ships stop critting through a Crit Rate Down, and Hacking/Security Down move how often debuffs land in both directions. Security Down also cuts damage that scales off security, such as Prophet\u0027s. As with every other family, if a ship already carries its own copy of the same debuff the stronger one wins rather than the two adding up.',
    "Two instances of the SAME buff or debuff no longer add up — the stronger one wins and the weaker is ignored, no matter which side put it there. This was already true within one side; it now holds across the line too. Say your ship is carrying its own Defense Down II (-30%) and the enemy hits it with a Defense Down III (-45%): it now sits at -45%, not the -75% the two used to sum to. The same fix covers Inc. Damage Up on a ship taking a hit, and the two repair channels — an Inc. Repair Down landing on the ship being healed, and an Out. Repair Down landing on the healer. Damage-over-time effects (Corrosion, Inferno) and bombs are the one exception and still stack, exactly as before. Different families are unaffected: an Attack Down and an Out. Damage Down are two different things and still combine, and so do a Defense Down and a Defense Shred. Where this shows up in practice is the manual buff pickers — nothing in any ship's kit puts the same named buff on both sides, but you can, by ticking it in the attacker list and the enemy-debuff list at once.",
    'Defense calculator: the headline is now ROUNDS SURVIVED and DAMAGE ABSORBED, and a defensive ability on the ship itself no longer LOWERS the second one through its own damage reduction — it raises it whenever it buys the ship another round. Damage absorbed means everything thrown at the ship over the fight — direct hits, damage over time, bombs, detonations and reflected damage — counted BEFORE the ship reduced any of it. Everything the DEFENDER does to shrink an incoming hit is deliberately left out of it: Defense mitigation, its own Inc. Damage Down, squad-leader incoming protections, gear damage reduction, and block procs. What the ATTACKER brings stays in, including Out. Damage Up and Exposed, because those make the attack bigger rather than making the defence better. Shields and Barrier still count too — and so does Shield Converter — because those eat damage that arrived, they do not stop it being thrown. Three things are worth knowing before you read a defensive ability as broken. First, that ability raises the figure only by buying the ship one more incoming HIT: if it was already surviving the whole window, or if it still dies to the very same hit, the number does not budge. Second, DEBUFFING THE ATTACKER lowers it. An Attack Down or Out. Damage Down your ship puts on the enemy shooting at it now shrinks what that enemy throws, so less arrives and the figure falls. Measured over four rounds against one 10,000-attack enemy: 40,000 absorbed with no debuff, 30,000 with your Attack Down standing on the enemy at -25%, 20,000 at -50%, and 4,000 at -90%; Out. Damage Down behaves identically, and an Out. Damage UP on the enemy pushes it the other way to 60,000. That is an engine change rather than a page change — the page merely made it visible — and it has its own entry below. Third, on the ALLY side: damage an ally soaks for the ship through Protection is booked against that ally, so adding a protector lowers the figure for this ship (a 30% redirect over four rounds takes 40,000 down to 28,000). The slice is not lost, it just shows up on the ally card instead of this one. Concretely, in a fight: give a 100,000 HP ship 5,000 Defense and put it under 60,000 damage a round. It dies on round 5 and reports 300,000 absorbed. Now hand that same ship Inc. Damage Down II (-30% incoming direct damage). It survives a SIXTH round — and used to report 252,000, LESS than the unprotected ship managed, because the buff was still being subtracted from every single hit on the way into the total. Six rounds of 60,000 with 30% shaved off each one comes to 252,000, so the discount more than cancelled out the extra round the buff had bought: protecting the ship LOWERED its score. It now reports 360,000, which is six rounds of the same 60,000. Reflect was doing the same thing on its own channel: a ship swinging into a 50%-reflect enemy took 200,000 of thorns over four rounds, but giving it a 30% incoming-damage-reduction passive dropped the reported figure to 140,000, and a 60% one to 80,000 — same four rounds, same thorns, a quarter of the score for being tougher. All three now read 200,000. A ship carrying Voron\u2019s damage-to-DoT transform was hit hardest of all: at 5,000 Defense it read 24,993 where a ship with no such passive read 100,000, so the most defensive kit on the page scored a quarter of the plainest one. Both now read 100,000, because the same amount really was thrown at both. Two things worth knowing when reading it. The figure moves in whole HITS, not whole rounds: it only grows when the ship survives one more incoming hit, never by the sliver a reduction shaves off each one. Against a single attacker a hit IS a round, so two such ships that die on the same round do report the same total even if one is a little tankier. Against several attackers a round is several hits, and the resolution is finer than a round — a reduction can carry the ship past the first attacker’s hit so that it also eats the second before dying, and the fight ends with the turn that destroys it, so the frailer ship never sees that second hit. Measured on a 100,000 HP ship at 5,000 Defense under two 40,000-attack enemies: it dies on round 4 having absorbed 280,000, and with Defense Up II it still dies on round 4 but absorbs 320,000 — one extra hit on the same round of death. Deliver that identical pressure with a single 80,000-attack enemy instead and both read 320,000. Rounds survived now sits above the figure so you can tell the two cases apart. And on a ship that SURVIVES the whole window, the figure describes the ATTACKERS rather than the ship: nothing killed it, so it is simply everything the enemy managed to throw, and two survivors that both last the FULL window under the same enemies tie no matter how differently tanky they are — as long as NEITHER of them kills an attacker. Its own offence is the one thing that separates two survivors, and it separates them DOWNWARDS: the harder-hitting ship is thrown less. It can end the fight outright — wipe the enemy team on round 6 of a 20-round window and the run stops there, so 14 rounds of fire are never thrown, and the card now says "the enemy team was wiped on round 6 of 20" instead of "survived all 6 rounds". Or it can just thin the volley while still standing the whole window: against two 5,000-attack enemies over six rounds, a ship with no attack absorbed 60,000, one that killed a single attacker part-way through absorbed 40,000, and one that killed it sooner absorbed 30,000 — all three survived all six rounds. If you are comparing survivors, raise enemy attack or add rounds until the ships actually die. The old static estimate is still shown beside them, renamed Theoretical EHP and captioned as what it is: a hangar-stats calculation from HP and Defense alone, with no enemy firing at it. The breakdown (to hull, absorbed by shield, blocked by Barrier, converted to shield) sits between the measured figures and Theoretical EHP, and still describes what actually reached the ship after everything it does to shrink an incoming hit — a different axis from the headline, not a sum of it — and its rows are now rounded like every other figure on the card instead of printing values like 24,999.667. One knock-on worth expecting: the "Best ship configuration" badge is now decided by damage absorbed rather than by the old static estimate, so for the very same inputs a different card can end up highlighted, and the "Compared to best" percentage beside it is measured on damage absorbed too (it used to compare Theoretical EHP against a best chosen on a different figure, which could show a card as worse in red while its number was actually higher). When the measured figures genuinely tie — no enemy pressure at all, or every configuration surviving the whole window — the badge falls back to Theoretical EHP, so the page you first land on still marks a best on the old static estimate instead of marking none. Rounds survived is the last resort after that rather than the first, because damage absorbed already grows with rounds, so rounds only ever speak when it ties — and with no enemy configured they speak backwards: the fight ends when a ship destroys the practice target the page stands in, which makes the hardest hitter show the FEWEST rounds. With rounds ahead of Theoretical EHP the badge went to the weakest-attacking ship on the very first page you see.',
    'Combat simulator and calculators: an Attack Down or Out. Damage Down that YOUR ship lands on an enemy now actually reduces what that enemy throws. Until now the engine honoured only the outgoing modifiers an attacker had put on ITSELF and silently dropped the ones its target had landed on it — the debuff was applied, it stood on the enemy card for its full duration, and every hit that enemy made still landed at full size. Measured on the Defense calculator’s four-round window against one 10,000-attack enemy: 40,000 damage thrown with no debuff, and an unchanged 40,000 with your Attack Down standing on that enemy at -90%. It now reads 30,000 at -25%, 20,000 at -50% and 4,000 at -90%, and Out. Damage Down behaves identically. Seventeen ships inflict one of these somewhere in their kit: twelve inflict Attack Down (APEX, Arum, Bayah, Bizon, Chisel, Cinya, Curator, Nayra, Nemesis, Opal, Prospect and Shepherd) and nine inflict Out. Damage Down (APEX, Anemone, Arum, Bayah, Curator, Kinetik, Vindicator, Voron and Warden), four of them both — and every one of those debuffs was doing nothing to the damage coming back at you. Five of the seventeen inflict it from a PASSIVE rather than from a skill you fire: Arum, Bayah, Opal, Shepherd and Warden. Only the refit-active passive applies, so Opal, Shepherd and Arum inflict theirs on either of their first two passives, while Bayah’s and Warden’s need the second passive active. In a fight: your Opal takes a hit, its passive puts Attack Down II (-30% Attack) on the attacker, and that attacker’s next shot really does land 30% smaller — previously it landed at full size and the debuff sat on the enemy card doing nothing. Enemy ships are held to the same rule against you: an enemy Curator’s Attack Down III now genuinely shrinks what your ships hit for. One rule to know, and it is the long-standing rule for buffs and debuffs generally rather than something invented here: two instances of the SAME named debuff do not add up, the stronger one simply applies. An enemy already carrying a -15% Attack Down I that you hit with a -45% Attack Down III throws at -45%, not -60%, and that holds whichever side applied the stronger one — if the enemy’s own is the stronger, yours is the one that is shadowed. Two DIFFERENT debuffs still combine: an Attack Down and an Out. Damage Down on the same enemy both apply, which takes that same 40,000 window down to 12,100. Damage over Time is untouched, as it always has been — it ignores these channels. Two paths still miss it and are filed as follow-ups: a counter-attack and a “when this happens, hit back” reactive proc are still thrown at full strength by a suppressed owner.',
    "The Defense calculator now runs the real combat engine using each ship's own skills, so shields, Barrier and self-repair all fire for real, and a buff with a condition on it is now held back while that condition is unmet — a Defense buff gated on the ship's own HP dropping below a threshold no longer counts while the ship is still above it. The old formula estimate is still shown beside it as Theoretical EHP, and that one does NOT model the gate: it reads the buff out of the skill text and counts it as permanently active, so it runs high on a ship like Redeemer whose Defense Up II only arrives below 60% HP. You can configure enemy attackers, supporting allies and the length of the fight — the enemy roster, the ally roster and the shared settings each sit in their own collapsible card. Each ship card also has a Show Advanced section with the same skill editor the DPS and Healing calculators use, so you can read the abilities parsed out of a ship's active, charged and passive skills and add or correct them by hand. The Passive row shows up whenever the ship HAS passive skill text, even when the parser read no abilities out of it — purely defensive and repair passives often parse to nothing, and on this page those are exactly the ones worth entering yourself.",
    'Combat simulator and calculators: a ship\u2019s own Defense buff now actually reduces the damage it takes \u2014 and a Defense penalty it gives itself now actually costs it. Defense Up I/II/III (+15/+30/+45% Defense) landed on the ship and showed in its buff list, but did nothing on the large majority of hits a ship takes \u2014 the direct hits landing on it each round. (It already worked on a counter-attack, a reactive proc and the Protection cascade\u2019s fallback read; those were never affected.) The main damage step read each defender\u2019s Defense straight off the ship sheet and then applied only the Defense changes an ENEMY had inflicted, so Defense Shred worked there and your own Defense Up did not. In a fight: put a ship with 5,000 Defense in front and let an enemy hit it for 8,331 a round, then give that ship Defense Up II (+30% Defense) \u2014 from its own kit, a gear buff, or the buff picker on the defense calculator \u2014 and that same hit now lands for 7,064 instead of the unchanged 8,331 it used to. Redeemer is a concrete case: its first passive gives it Defense Up II the moment its own health drops below 60% (Defense Up III on the second), and until now that buff did nothing against the hits actually landing on it. The same fix works in the other direction, which is the bigger change in practice. Overload (+10% Outgoing Direct Damage, -10% Defense, stacking to ten times) is carried by Butcher, Mangler, Ravager, Asphyxiator and Ruiner, and the app was granting its damage bonus while quietly ignoring the Defense cost printed on the same card \u2014 which made those five ships strictly better than they read. A Butcher gaining a stack every turn now gets steadily easier to kill as it winds up: 4,500 Defense after one turn, 2,500 after five, and from the tenth turn on it defends as though it had none at all. Defense cannot usefully go past that point \u2014 zero is the floor a ship\u2019s own Defense can push itself down to, so a fully-stacked Butcher takes the plain, undefended hit, never more than that. Refine\u2019s Supercharged (-20% Defense on the active, -60% on the charged) costs it the same way for as long as it stands. Enemy ships follow every one of these rules against you: an enemy holding Defense Up is genuinely harder to chew through, and an enemy Butcher at full Overload folds faster. Damage over Time ticks are unaffected, as always \u2014 they ignore Defense entirely. The defense calculator and the survivability figures on it were reading the same engine, so they change with it — though note WHICH figures move there: Defense mitigation is deliberately excluded from that page’s Damage absorbed headline, so a Defense buff shows up in Rounds survived, in the "Reached the ship" sub-total and in Theoretical EHP, and only reaches the headline insofar as the extra hits it survives — a whole extra round, or just one more attacker’s hit on the round it dies — bring more incoming fire with them.',
    ...
    "Defense calculator: the page is faster to use. Renaming a ship configuration no longer re-runs the fight simulation for every ship on the page — it used to, because the recalculation was watching the whole configuration including its name. And typing into a ship's HP, Defense or Security field now waits briefly after you stop typing before recalculating, instead of re-running the simulation on every single digit. Typing a number no longer makes the page stutter.",
    'Defense calculator: Theoretical EHP no longer counts a kit buff whose condition it cannot confirm is met, and the card now names what it left out and why. HP-based gates are checked against a ship at full health: Redeemer at 40,000 HP and 5,000 Defense used to read 113,250 — that total included Defense Up II, which only arrives once Redeemer drops below 60% HP — and now reads 96,025 at full health, with a line under the figure reading "Not counted (conditional): Defense Up II - below 60% HP". Gates that depend on your team or the enemy are checked against what you have configured on this page instead of being dropped outright: Chakara’s "lowest Speed among allies" now counts as soon as no ally in your team roster is as slow or slower (an empty team roster counts, since Chakara is trivially the slowest of one), and Asphyxiator/Bayah’s "enemy has N+ debuffs" now counts once your team roster is configured to land that many distinct debuffs on a configured enemy. A gate this figure genuinely cannot answer (Taunt/Provoke status, a live crit, and similar) still gets held back exactly as before. Buffs you picked yourself in the buff picker are unaffected; this only changes buffs auto-filled from a ship’s own kit. Because of the roster-dependent gates, Theoretical EHP is no longer a pure hangar-stats number — changing your team or enemy setup can move it. One knock-on: on a page with no enemies configured, where "Best ship configuration" and the chart’s ship markers both fall back to Theoretical EHP because there is no measured fight to rank by, the badge can land on a different card than before for the exact same inputs — both read the same, now more honest, figure.',
    'Defense calculator: picking a ship whose skills say it starts combat fully charged now actually starts it charged in the simulation. Akula, Chimei, Los, Sansi, Valkyrie and Wusheng all declare this, and the page was reading their charge COUNT off their kit while ignoring the charged-AT-START part — so their charged turn never landed on round 1 the way it does for the same ship picked as an enemy or a teammate elsewhere on the page. Pick Akula as the defender now and it takes its charged turn on round 1 like it does in game, and Rounds survived and Damage absorbed rise for all six ships to match.',
    'Defense calculator: the ship card’s Show Advanced section now has a Charge Count field and a Start Charged checkbox, the same controls already on the enemy roster, team roster and healer cards. The entry above made the defender honour a ship’s own "starts combat fully charged" kit text automatically, but the card gave you no way to see that it had happened or to turn it off — picking Akula, Chimei, Los, Sansi, Valkyrie or Wusheng silently started them charged with nothing on screen showing it. You can now see the detected charge count and starting state for any ship you pick, and override either one — useful for testing a ship without its usual first-turn charge, or for a manually-configured card with no ship attached at all, where Charge Count previously had no way to be set above 0.',
    'The DPS calculator now runs the full combat engine. Ships whose damage comes from a shield or repair-leech basis — or whose skills are gated on their own HP — previously showed no contribution at all in the DPS calculator; they now report their real output.',
    'Community recommendations: builds shared before the newer, fuller format are no longer silently dropped from the list. Older rows recorded a ship role as free text (e.g. "DEBUFFER (Corrosion)" or just "CORROSION") instead of a fixed role key, sometimes recorded a gear set with no piece count, sometimes recorded a stat bonus in an older shape, and sometimes recorded a stat priority or stat bonus by its on-screen name (e.g. "crit rate", "crit power", "heal modifier", "defense") instead of its internal key — any one of these caused the whole recommendation to disappear from the panel with no visible error. Role text is now matched to the right role automatically, a gear set with no recorded piece count now shows just the set name instead of vanishing, the older stat-bonus shape is now read correctly, and a stat recorded by its display name is now matched to the same stat automatically. Applying a build with no recorded piece count for a gear set uses 2 pieces, and the confirmation dialog says so before you apply it.',
];

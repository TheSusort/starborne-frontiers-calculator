...
export const UNRELEASED_CHANGES: string[] = [
    ...
    'DPS Calculator conditions now support count thresholds — e.g. "if the target has 3 or more debuffs" or "if this unit has no debuffs". Auto-detected from skill text and editable per condition (at least / at most / exactly N), so threshold-gated effects fire faithfully instead of on the first buff/debuff.',
    'DPS Calculator skill auto-fill now recognises buffs and debuffs phrased with more verb forms — "inflict"/"inflicting"/"is inflicted with", "apply"/"applying"/"is applied with", and "gaining" — so debuff applications that previously went undetected now populate the ability editor. Auto-filled debuffs are also tagged Inflict or Apply based on the skill verb. This now drives the simulation: Inflict debuffs are resistible (they roll against your Hacking vs the enemy Security each round), while Apply debuffs are guaranteed to land — except at an affinity disadvantage, where (matching the game) they are resisted.',
    'DPS Calculator skill auto-fill now detects conditional damage bonuses gated by enemy class — e.g. Meiying\'s "when attacking a Supporter, it additionally deals 90% damage". The bonus is added to the base skill multiplier only when the simulated enemy matches that type.',
    'DPS Calculator skill auto-fill now gates buffs granted "when Damaging a Debuffed enemy" (e.g. Sha Xing\'s Tianchao Precision), and "when/on applying or inflicting a debuff" (e.g. Yuyan, Marauder Rage), on the enemy actually having a debuff, instead of treating the buff as always active.',
    'DPS Calculator now shows the Passive skill slot whenever a ship has passive skill text, even when nothing was auto-detected from it (e.g. defensive/repair passives like Anemone). You can open it to read the skill text and add abilities manually.',
    'DPS Calculator skill auto-fill: fixed "deals N% damage to enemies below X% HP" skills (e.g. Judge) — the damage is now correctly gated on an enemy-HP threshold instead of being mislabelled with a per-destroyed-enemy scaling that belonged to a separate "% more damage" passive.',
    ...
    'DPS Calculator skill auto-fill now recognises enemy-type gates phrased "when attacking a <class>" (e.g. IonScorp) and count-threshold gates phrased "after dealing damage to an enemy with N debuffs" (e.g. Bayah, Bizon).',
    'DPS Calculator HP-threshold conditions can now target the unit\'s own HP, not just the enemy\'s — so buffs gated on "if it is at full HP" (e.g. Cobalt) are recognised. The editor adds a Self/Enemy selector to HP conditions.',
    'DPS Calculator skill auto-fill now parses "increases outgoing direct damage by up to N%" (e.g. Akula) as a flat +N% outgoing-damage modifier — the HP-scaling bonus is at its max since the sim treats the enemy as full HP.',
    'DPS Calculator skill auto-fill now models "if critical, additionally deals N% damage" (e.g. Crucialis) as a self-crit conditional bonus — the base damage always applies and the extra N% is added on a crit (weighted by crit rate).',
    'DPS Calculator skill auto-fill now classifies Taunt/Provoke targeting gates correctly — "when affected by Taunt or Provoke" (e.g. Thresh) maps Taunt to a buff on the enemy and Provoke to a debuff on this unit, instead of treating both as self-buffs.',
    'DPS Calculator skill auto-fill now gates "X% more damage when its HP is below Y%" modifiers (e.g. Los) on a self HP-threshold, instead of applying the bonus unconditionally. It also parses "increases Damage by N% to enemies with less than Y% HP" (e.g. Obsidian) as an outgoing-damage modifier gated on enemy HP.',
    'DPS Calculator skill auto-fill no longer mis-applies a conditional gate to a recurring per-turn grant in the same sentence — e.g. Shashou\'s "gains Stealth after damaging a Debuffer/Supporter and gains 1 stack of Blast each turn" now gates only Stealth, with Blast stacking unconditionally every round.',
    'DPS Calculator now models crit-power DoT extensions: skills that "extend … by N turns with the chance equal to Critical Power" (e.g. Valerian, Belladonna) auto-fill as an extend-DoT with a "Chance = Crit Power" option, rolling min(1, Crit Power/100) and gated by their trigger (self crit, or an ally inflicting). Valerian\'s self-crit Corrosion extension affects its DPS; team-triggered ones are represented for the editor. Crocus\'s "when an ally crits with a DoT, inflict Corrosion" is parsed as a DoT gated by a new "when an ally crits with a DoT" condition.',
    ...
    'DPS Calculator now models Echoing Burst-style debuffs (e.g. Valkyrie\'s Charged Skill) — while active they gather all direct damage dealt to the enemy, then detonate for that accumulated total when they expire. Auto-detected from skill text and editable as a new "Accumulate & Detonate" ability type, with the burst counted as Detonation damage.',
    'DPS Calculator skill auto-fill: fixed a condition leaking onto the wrong effect when a buff/debuff name contains an abbreviation period — e.g. Asphyxiator\'s "Inc. DoT Damage Up III" was wrongly gated by the "3 or more debuffs" condition that belongs only to the Stasis it inflicts. Such effects are now correctly recognised as unconditional, with the condition scoped to its own sentence.',
    ...
    'DPS Calculator skill editor: the Passive skill button now appears for un-refit ships too (e.g. FrontLine) — the base passive is innate, so it\'s no longer hidden until the ship is refit. Also stopped mis-reading Shield-gain text like "Shield equal to 25% of Max HP" as bonus HP-based damage.',
    'DPS simulator overhaul: crits now follow a deterministic per-round schedule instead of an averaged multiplier (crit rounds shown with a "Crit" badge in the chart tooltip), ability conditions actually gate damage, DoTs, and detonations so unmet conditions contribute nothing that round, enemy HP declines as damage accumulates so "below X% HP" execute-style effects kick in mid-fight (the chart tooltip shows the enemy\'s remaining HP% each round, and each ship\'s line is marked with a ringed dot on the round its damage empties the enemy HP pool), passive-skill damage now fires as an extra hit when its conditions pass (e.g. Judge\'s start-of-round 60% against enemies below half HP), HP-proportional bonuses now track the enemy\'s live HP instead of assuming the maximum (Akula\'s "up to 30% based on current HP" decays as the enemy drops; Tithonus\' missing-HP bonus ramps up), and results are fully reproducible run-to-run. Skill abilities now execute in skill-text order and can be reordered in the editor.',
];

...
export const UNRELEASED_CHANGES: string[] = [
    'Recruitment calculator: affinity weighting corrected to match the game (Thermal/Chemical/Electric form one combined pool at 10x the pull weight of antimatter ships), with stackable 10x/20x faction event multipliers.',
    'DPS Calculator skills are now edited through a new ability editor: Active / Charged / Passive slots whose abilities (damage, secondary damage, conditional scaling, charge gain, DoTs, buffs, debuffs, modifiers) are auto-filled from skill text and fully editable in one place, replacing the old separate config sections. Abilities can be reordered, the modal has shortcuts between slots, and the Passive slot always shows when a ship has passive text. Re-selecting a ship rebuilds its abilities from skill text, replacing manual edits.',
    'DPS simulation models many more skill mechanics: secondary damage scaling off Defense or max HP, count-scaled conditional bonuses (per debuff, per adjacent ally, …), charge manipulation with an enemy-type selector, DoT extension (including crit-power-chance and passive-sourced), DoT detonation and Echoing Burst-style accumulate-and-detonate (grouped as "Detonation" damage, replacing the "Bomb" line), attacks that cannot crit, and passive-skill damage hits (e.g. Judge\'s start-of-round 60% vs enemies below half HP).',
    'DPS skill auto-fill understands far more skill text: enemy-class gates (incl. negated and "X or Y" forms), crit gates, Taunt/Provoke status gates, count thresholds ("3 or more debuffs", "no debuffs"), self and enemy HP thresholds, "to enemies afflicted with <effect>" gates, team-dependent conditions (ally inflicts a debuff / is critically repaired / crits with a DoT / a specific ally is on the team), more buff and debuff verb forms, flat and per-buff defense penetration, "% more critical damage", enemy-class damage bonuses (e.g. Meiying), and crit-conditional bonus damage (e.g. Crucialis).',
    'DPS debuff landing is modeled per application type: Inflict debuffs roll your Hacking vs the enemy Security each round, while Apply debuffs always land — except at an affinity disadvantage, where (matching the game) they are resisted.',
    'DPS simulator overhaul: crits follow a deterministic per-round schedule instead of an averaged multiplier (with a "Crit" badge in the chart tooltip), ability conditions actually gate damage, DoTs, and detonations, abilities execute in skill-text order, and identical inputs always produce identical results.',

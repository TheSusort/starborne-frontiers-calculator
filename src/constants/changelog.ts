...
export const UNRELEASED_CHANGES: string[] = [
    ...
    "DPS calculator and healing mode: a multi-hit skill's own outgoing reactions now split across each of its separate attacks, matching the battle simulator. Burner's Inferno now stacks once per attack (three times on Enforcer's three-hit active, not once), and a Bloodthirst-style self-repair now scales off each attack's own damage instead of the whole cast's total — the DPS calculator and healing mode were previously over-crediting that repair threefold on a three-hit skill.",
];

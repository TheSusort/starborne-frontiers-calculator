...
export const UNRELEASED_CHANGES: string[] = [
    'Combat Simulator: the “Healing done” figure now counts every kind of repair, not just cast repair skills. A ship that repairs itself off the damage it deals (Magnolia, Valerian, the Leech gear set) or that repairs when it is hit (Isha) used to show the repair under “Healing received” with “Healing done” sitting at 0 beside it — the HP visibly came back and nobody was credited for it. Both figures now agree, on your ships and the enemy\u2019s alike. Repair Over Time ticks are deliberately still excluded from “Healing done”: a tick is not a repair the holder performed.',

...
export const UNRELEASED_CHANGES: string[] = [
    ...
    'Healing calculator and combat simulator: a passive that repairs or shields for a share of the damage the ship deals now also pays out on damage-over-time ticks. Magnolia has one, and the Leech gear set gives every ship the same thing — but it only ever paid out on the damage from the attack itself, so all the Corrosion and Inferno damage ticking away on the enemy healed nothing. It now pays out on those ticks too, and an enemy ship with the same passive repairs itself off its own damage-over-time in the same way.',
];

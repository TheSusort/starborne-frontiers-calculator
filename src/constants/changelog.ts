...
export const UNRELEASED_CHANGES: string[] = [
    'DPS calculator: the target is now a real enemy ship that fights back. You can pick an actual ship for it (filling in its stats and skills) or set them by hand, and it takes its own turns and attacks you instead of standing there. Ships built around being hit — counterattacks, reflects and on-hit triggers — were understated before, because nothing ever struck them; they now show their real damage. Your ship can be destroyed too, which ends the run. Each config and team ship also picks the board slot it fights from, which affects targeting patterns and adjacency. Existing numbers will shift, since fights now play out both ways.',

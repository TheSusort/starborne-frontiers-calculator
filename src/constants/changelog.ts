...
export const UNRELEASED_CHANGES: string[] = [
    ...
    "Combat simulator: a leech now pays off what the hit actually did, not what it was thrown for. Two cases change. A hit a Voron, Orel or Hit-Mitigation ship converts into a Damage over Time effect pays the attacker's leech NOTHING — the damage isn't dealt this turn, so a Magnolia hitting one of them used to be repaired for a hit that never landed. And a leech reading the damage a ship TOOK now counts only the ship's own share: with Meatshield's Protection redirecting 30% of a hit onto Meatshield, Malvex's and Quixilver's shields are sized off the 70% they actually took, not the whole incoming hit. Leeching off damage DEALT is unchanged when Protection fires — the attacker still dealt the protector's chunk plus whatever was left on the target, and counts both.",

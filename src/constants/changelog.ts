...
export const UNRELEASED_CHANGES: string[] = [
    'New Combat Simulator: place two squads on the hex board and watch a round-by-round battle play out — both sides are your own fleet ships using their geared stats. Step through each round to see per-ship HP, damage, and healing on the board, pin a ship for a detailed per-round breakdown, and read the turn-by-turn event log — now a chronological play-by-play with turn markers, team-labeled (enemy ships are prefixed with "Enemy") and color-coded, showing who hit whom for how much alongside heals, buffs, debuffs, damage-over-time afflictions, and deaths — with the final outcome called out at the top.',

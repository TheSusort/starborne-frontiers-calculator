...
export const UNRELEASED_CHANGES: string[] = [
    "Combat simulator and calculators: a ship whose skill targets its own allies now faces no enemy at all on that turn, instead of being handed an invisible stand-in opponent. Its repair, shield or buff is unchanged, but every question the turn asks about the enemy — is it shielded, how much HP does it have, what is its Defense — is now answered with \u201cthere is no enemy\u201d rather than with a healthy stand-in's stats. One visible consequence: repairs and shields from these ships are now labelled in the combat log with the skill that produced them (AEGIS's charged Barrier, Hermes's and Mender's repairs), because the stand-in was previously opening an empty attack line that claimed the label and was then discarded. Support skills also no longer set off allies' \u201cwhen an ally critically hits an enemy\u201d passives, since a repair is not a hit \u2014 the matching \u201ccritically repairs\u201d passives are unaffected.",

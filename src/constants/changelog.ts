...
export const UNRELEASED_CHANGES: string[] = [
    ...
    "Combat simulator: effects that trigger on dealing damage (Burner's Inferno, Warpstrike's debuff-duration reduction, Zeolite's purge) no longer fire off an attack that landed for nothing. They were gated on the damage figure shown before the target's defences were applied, so an attack fully soaked by a Shield, fully stopped by an incoming-block ability, or entirely converted into a Damage over Time effect (Voron, Orel) still paid out in full. They now read what the attack actually delivered. Damage redirected to an ally by Protection still counts, since that hit did land. Both sides are affected. The DPS and healing calculators are unchanged — nothing there can soak or convert a hit, so the two figures never disagree.",
];

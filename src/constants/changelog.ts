...
export const UNRELEASED_CHANGES: string[] = [
    ...
    "DPS calculator and healing mode: a multi-hit skill's own outgoing reactions now split across each of its separate attacks, matching the battle simulator. Burner's Inferno now stacks once per attack (three times on Enforcer's three-hit active, not once). Healing mode also gets two further corrections: a Bloodthirst-style self-repair now scales off each attack's own damage instead of the whole cast's total, and reactions to an ally's critical hit (such as banking extra charge) now fire once per critting attack instead of once for the whole cast — a 2-hit critical skill now correctly grants 2 charges, not 1.",
];

...
export const UNRELEASED_CHANGES: string[] = [
    ...
    "Combat simulator: Exposed, Toxic Overflow and Hit Mitigation are one-shot statuses — each is used up by the single hit, or single end of round, that reads it. Picking one by hand in a buff or debuff picker now has no effect, because a hand-picked status carries no turn count and so could never be used up: a hand-picked Toxic Overflow used to spread Corrosion to adjacent allies every round for the whole fight instead of once. They still apply in full when a ship's skill grants them.",

...
function renderFinding(finding: Finding): string {
  ...
  if (finding.fingerprintDiff) {
    ...
    lines.push(`- Missing in composition: ${finding.fingerprintDiff.missingInComposition.join(', ')}`);
    lines.push(`- Extra in composition: ${finding.fingerprintDiff.extraInComposition.join(', ')}`);
    lines.push('');
  }
 ...
 */
export function writeLedger(findings: Finding[], meta: LedgerMeta, outDir: string): void {

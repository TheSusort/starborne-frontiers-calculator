...
CREATE TABLE public.users (
  ...
  username text UNIQUE CHECK (username IS NULL OR length(username) >= 3 AND length(username) <= 20 AND username ~ '^[a-zA-Z0-9 _-]+$'::text),
  is_public boolean DEFAULT false,
  in_game_id text,
  tutorial_completed_groups ARRAY DEFAULT '{}'::text[],
  owner_auth_user_id uuid,
  CONSTRAINT users_pkey PRIMARY KEY (id),
  CONSTRAINT users_owner_auth_user_id_fkey FOREIGN KEY (owner_auth_user_id) REFERENCES auth.users(id)
);
...
CREATE TABLE public.community_recommendations (
  ...
  stat_priorities jsonb DEFAULT '[]'::jsonb CHECK (stat_priorities IS NULL OR octet_length(stat_priorities::text) <= 65536),
  stat_bonuses jsonb DEFAULT '[]'::jsonb CHECK (stat_bonuses IS NULL OR octet_length(stat_bonuses::text) <= 65536),
  set_priorities jsonb DEFAULT '[]'::jsonb CHECK (set_priorities IS NULL OR octet_length(set_priorities::text) <= 65536),
  reasoning text,
...
END,
  ...
  shared_config jsonb CHECK (shared_config IS NULL OR octet_length(shared_config::text) <= 65536),
...
CREATE TABLE public.arena_season_rules (
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT arena_season_rules_pkey PRIMARY KEY (id),
  CONSTRAINT arena_season_rules_season_id_fkey FOREIGN KEY (season_id) REFERENCES public.arena_seasons(id)
);
CREATE TABLE public.statistics_snapshots (
  ...
  CONSTRAINT statistics_snapshots_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
CREATE TABLE public.gear_wishlists (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  entries jsonb NOT NULL DEFAULT '[]'::jsonb,
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT gear_wishlists_pkey PRIMARY KEY (id),
  CONSTRAINT gear_wishlists_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
CREATE TABLE public.autogear_teams (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  name text NOT NULL,
  ship_ids jsonb NOT NULL DEFAULT '[]'::jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT autogear_teams_pkey PRIMARY KEY (id),
  CONSTRAINT autogear_teams_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
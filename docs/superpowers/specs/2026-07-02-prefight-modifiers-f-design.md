...
- **`src/utils/combat/playerTurn.ts`**: new optional `preFight` arg; after `resolveSelfBuffTotals` (~:947) fold `outgoingDamage`→outgoingDamageBuff, `outgoingCritDamage`→critDamageBuff, `outgoingHeal`/`incomingHeal`→heal buffs. Flows through `effectiveDamageStatsOf` into every existing damage/heal consumer; enemy attackers walk the same path (symmetry).

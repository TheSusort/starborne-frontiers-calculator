...
For each `teamActors[i]` with `shipSkills` (decisions locked):
- `debuffLandingChance`: its `stats.hacking` vs `enemySecurity`, with ITS affinity damage modifier (same formula as the attacker's).
- `selfDotModifier`/`defensePenetrationBuff`: the team actor's own rates start at `0` — its walked statuses produce these in-loop (matching the attacker's in-loop ability fold semantics). The attacker's static fold over the MANUAL lists (`toDotAndPenModifiers(selfBuffs, [])`) is attacker-granted and stays on the ATTACKER's rates exactly as today.
- Affinity modifiers: add optional `enemyAffinity?: AffinityName` to `DPSSimulationInput` (additive) and derive per-team-actor modifiers IN THE ADAPTER via the same `computeAffinityModifiers` helper the page uses (`src/utils/calculators/affinityUtils.ts`). When `enemyAffinity` or `t.affinity` is absent, team affinity modifiers default to 0/100/0. The page passes `enemyAffinity` in Task 9.

...
**⚠️ CONSUMPTION MODEL (corrected — read before implementing).** Task 6 made Cheat Death a `'recurring'` (always-active) buff: it is re-derived from ship data each round and is NOT stored in the StatusEngine's timed maps. Therefore you CANNOT "consume" it by deleting it from a store — it would regenerate next round, and `clearRemovable` (which sweeps only the timed maps) doesn't touch it. **Consumption is a per-actor engine flag**, not a store mutation. Track a combat-lifetime `Set<actorId>` (e.g. `cheatDeathConsumed`) owned by the engine outside the round loop. The intercept fires iff: the actor carries a `CHEAT_DEATH_BUFFS` name in its active buff snapshot AND its id is not in `cheatDeathConsumed`. On fire, ADD the id to the set. This gives the canonical **once-per-combat-per-actor** semantic (re-grant refresh is moot: an always-active buff is never "absent", so "until triggered + once per combat" = fires once per combat — matches the user's primary constraint; document this).

**Ordering (spec, canonical):** HP would reach 0 → **(1)** carrier has Cheat Death AND id ∉ `cheatDeathConsumed`? → HP = 1, add id to `cheatDeathConsumed`, `statusEngine.clearRemovable(actorId)`, emit `cheat-death-activated{actorId, round}`, STOP (no `recordDestroyed`). **(2)** else → `recordDestroyed` (Task 4) → `ship-destroyed` → death listeners.

- [ ] **Step 1: Write failing tests** (healing mode, tank carries Cheat Death — grant it via the tank's parsed kit OR by seeding a 'Cheat Death' recurring buff so `snapshot(tankId)` surfaces it):
  ...
  - **Consumed (flag):** a SECOND lethal hit destroys the tank normally (`ship-destroyed` + `destroyedRound`) — even though the recurring Cheat Death buff still appears in the snapshot, the `cheatDeathConsumed` flag prevents re-triggering.
...
- [ ] **Step 3: Implement.** Add an engine-owned `const cheatDeathConsumed = new Set<string>();` OUTSIDE the round loop (combat-lifetime). In `applyIncomingToTarget`, before the death block (which now calls `recordDestroyed`): read the target's active buff names via `statusEngine.snapshot(healTarget!.id)` — confirm the self-buff-names field against `statusEngine.ts` (the snapshot exposes active self-buff names; a `'recurring'` Cheat Death surfaces here). If a `CHEAT_DEATH_BUFFS` name is present AND `!cheatDeathConsumed.has(targetId)` → set `currentHp = 1`, `cheatDeathConsumed.add(targetId)`, call `statusEngine.clearRemovable(targetId)`, emit `cheat-death-activated{actorId: targetId, round: r}`, and `return` the intake result WITHOUT recording destroyed. Do NOT attempt to delete the buff from any store. Apply the same guard in the general death path if it's reachable for a Cheat-Death carrier (the enemy path is safe to leave as-is — the DPS enemy never carries Cheat Death; only add it there if trivially shared).
- [ ] **Step 4: Run green.** PASS.

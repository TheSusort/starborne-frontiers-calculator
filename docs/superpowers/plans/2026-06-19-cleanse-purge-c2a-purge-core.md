...
- **Emit ONLY from active/charged slots** (the over-approximation fix): every reactive/conditional purge in the corpus lives in a PASSIVE (Faust on-death, Iridium on-attacked, Rhodium end-of-round, Sefuba on-purged p1/p2 incl. the "purges 1 more" chain, Salvation on-ally-purged, Lodolite on-purged, and any similar passive-slot purges). Every genuine ON-CAST purge lives in an ACTIVE or CHARGED skill (Sefuba active, Cobalt active/charged, Chakara charged, Tithonus active, Zeolite active, Amartya charged). So gating the emit on `slot === 'active' || slot === 'charged'` cleanly excludes ALL reactive/conditional purges — no purge-trigger detection needed in C2a, and it eliminates the Sefuba-p2 double-emit. (`slot` is the per-slot discriminant in scope in the buildShipAbilities emission — `SkillSlot = 'active'|'charged'|'passive'`, all passives collapse to `'passive'` — as used by the cleanse block's `flipBareSupportTarget`.)
...
- **Conditional/non-on-cast purges** (Faust on-death, Iridium on-attacked, Rhodium end-of-round, and any other passive-slot purge) are NOT emitted in C2a because the emit is slot-gated to active/charged. C2b adds purge-trigger detection to fire them. **Amartya** (charged, "purges 1 buff from all enemies for every 50% crit power") IS emitted but C2a fires a flat count:1 against the single targetId — an under-approximation (never removes more than the game would), acceptable until E (AoE) + count-scaling land.

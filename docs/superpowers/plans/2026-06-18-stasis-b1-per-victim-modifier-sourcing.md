...
- [ ] **Step 2b (code-review follow-ups from Task 3):** (i) export `DEFAULT_ENEMY_TARGET` from `statusEngine.ts` and import it in `triggers.ts` (remove the duplicated module-private copy at `triggers.ts:697` — single source of truth); (ii) tidy the tombstone comment at the `expandBuffs`/`expandEnemyDebuffs` deletion site in `playerTurn.ts` (point to the import block or trim). Both byte-identical.
